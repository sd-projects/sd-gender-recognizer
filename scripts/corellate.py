...
for i, audio_data in enumerate(audio_dt_name):
    ...
    df.append([a, audio_data, tm , np.max(rolloff), np.mean(mfccsdf), np.sum(y_harmonic)/tm, np.sum(y_percussive)/tm,
               ...
               , np.var(y_percussive)*10**4, np.median(y_harmonic)*10**10, np.median(y_percussive)*10**10,
               np.sum(mfccs)/tm, np.mean(mfccs), np.var(mfccs)
               , np.sum(Xdb)/tm, np.mean(Xdb), np.var(X), np.var(Xdb), np.var(zero_crossing), np.mean(zero_crossing),
               np.sum(zero_crossing)/tm, np.median(zero_crossing)])
print(df)


df = pd.DataFrame(df, columns=[" sex", " audio_data", "time", "np.mean(contrast)","np.mean(mfccsdf)", "np.sum(y_harmonic)",
